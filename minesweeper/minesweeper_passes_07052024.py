...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for neighborsR in range(-1, 2):
            for neighborsC in range(-1, 2):
                if (neighborsR != 0 or neighborsC != 0):
                    if 0 <= neighborsR + row < self.height and 0 <= neighborsC + column < self.width:
                        neighbors.add((neighborsR + row, neighborsC + column))
        ...
        unknown_moves = unknown_moves.difference(self.moves_made)

        # take intersection so you can decrease the count by the amount of known mines
        decrease_count = len(neighbors.intersection(self.mines))

        # add new neighbor info
        if len(unknown_moves) > 0:
            self.knowledge.append(Sentence(unknown_moves, count - decrease_count))

        # update knowledge based on new sentence
        self.update_knowledge()

        # infer new sentences from knowledge base
        self.infer_knowledge()

    def update_knowledge(self):
        """Recursively updates the knowledge based on the new sentence"""

        # make a deep copy so don't mess up database
        ...
        for sentence1 in knowledge_copy:
            for sentence2 in knowledge_copy:
                ...
                if sentence1.cells.issubset(sentence2.cells) and sentence1.cells != sentence2.cells:
                    ...
                    # create a new sentence that is the differance between two sentences
                    ...
                    if len(infered_sentence.cells) > 0 and infered_sentence not in self.knowledge:
                        ...
                        # recursively call infer_knowledge to check knwoledge with infered sentence
                        self.infer_knowledge()
    ...
    def make_random_move(self):
        ...
        # put all moves into other moves
        for i in range(self.height):
            for j in range(self.width):
                other_moves.add((i, j))
