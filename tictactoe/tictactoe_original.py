...
#adapted from lecture 0's slideshow (slides #285, #286)
    ...
    def max_value(board):
        ...
        #check if board is terminal case
        if terminal(board):
            return (utility(board), None)

        v = float('-inf')
        for action in actions(board):
            minimum = min_value(result(board, action)) #call min_value to search deeper
            if minimum[0] > v:
                v = minimum[0]
                best = action
        return (v, best)
    ...
    def min_value(board):
        ...
        v = float('inf')
        for action in actions(board):
            maximum = max_value(result(board, action)) #call for max_value to search deeper
            if maximum[0] < v:
                v = maximum[0]
                best = action
        return (v, best)
    ...
    #call max_val or min_val first if player is X or O
    if player(board) == X:
        test = max_value(board)
        return test[1]

    if player(board) == O:
        test = min_value(board)
        return test[1]
